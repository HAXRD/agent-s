...
- [x] 创建数据库管理脚本 (`scripts/db.sh`)：
  ...
  - 实现 `stop` 命令：停止数据库
  - 实现 `restart` 命令：重启数据库
  - 实现 `status` 命令：查看数据库状态
  - 实现 `logs` 命令：查看数据库日志
  - 实现 `shell` 命令：连接到数据库命令行
...
- [x] 创建统一启停脚本 (`scripts/manage.sh`)：
  ...
  - 启动后端服务：
    - 检查环境变量配置
    - 检查数据库连接（可选，使用 `scripts/db.sh status`）
    - 后台启动后端服务（`go run cmd/server/main.go`）
    - 记录进程ID到文件（`server.pid`）
    - 输出启动日志
  - 启动前端服务：
    - 检查环境变量配置
  ...
  - 状态检查：
    - 检查后端服务运行状态
    - 检查前端服务运行状态
    - 检查数据库运行状态（使用 `scripts/db.sh status`）
    - 显示服务状态信息
  - 重启功能：
    - 调用停止功能
    - 等待服务完全停止
...
**P0任务总时间：** 约 42-47小时（阶段1增加1小时Docker配置）  
**P1任务总时间：** 约 6小时  
**总计：** 约 48-53小时
